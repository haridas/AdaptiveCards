...
export class FetchManager {
    ...
    async getSampleImages() {
        ...
        let sampleImages = await fetch(process.env.PIC_TO_CARD_PREDICTION_API+"/get_card_templates", option);
...
export class OpenImageDialog extends Dialog {
	...
	private _predictedCardJSON: JSON;
	...
	private geneareteErrorText(text) {
    ...
    private loadSpinner() {
        ...
        spinnerHostElement.style.position = "absolute";
        spinnerHostElement.style.backgroundColor = "black";
        spinnerHostElement.style.opacity = ".4";
        spinnerHostElement.style.height = "100%"
        spinnerHostElement.style.width = "100%";
...
export class ImageItem {
    ...
    render(): HTMLElement {
        const imagePlaceholder = <HTMLImageElement> document.createElement("img");
